...
namespace tcc = bitfusion::common::conversion;

using namespace bitfusion::runtime;

template <typename T>
Decoder<T>::Decoder(size_t vocabSize, size_t vocabSizePadded, CudaStreamPtr const& stream)
    : mManager{stream}
    , mAllocator{mManager}
{
    ...
    mDynamicDecodeLayer = std::make_shared<bitfusion::layers::DynamicDecodeLayer<T>>(
...
template <typename T>
void Decoder<T>::setup(SamplingConfig const& samplingConfig, size_t batchSize, SizeType maxSequenceLength)
{
    ...
    DecodingOutput& output, DecodingInput::TensorPtr const& inputLengths, DecodingOutput::TensorPtr& logProbsTiled)
{
    ...
    outputParams.beamHypotheses = std::make_shared<bitfusion::kernels::BeamHypotheses>();
...
template <typename T>
bool Decoder<T>::forward(DecodingOutput& output, DecodingInput const& input)
{
    LOG_DEBUG("%s start", __PRETTY_FUNCTION__);
...
template <typename T>
void Decoder<T>::forwardAsync(DecodingOutput& output, DecodingInput const& input)
{
    LOG_DEBUG("%s start", __PRETTY_FUNCTION__);
    auto forwardParams = prepareInputs<T>(input);
    auto outputParams = prepareOutputs<T>(output, input.lengths, mLogProbsTiled);
...
template <typename T>
void Decoder<T>::gatherTree(ITensor& finalOutputIds, DecodingOutput const& decodingOutput,
    DecodingInput const& decodingInput, BufferManager const& manager)
{
    ...
    bitfusion::kernels::invokeInitializeOutput(bufferCast<TokenIdType>(finalOutputIds),
        bufferCast<TokenIdType>(*decodingInput.endIds), batchSize * beamWidth, maxSeqLength, stream.get());
    sync_check_cuda_error();

    bitfusion::kernels::BeamHypotheses beamHypotheses;
    ...
    bitfusion::kernels::invokeInsertUnfinishedPath(beamHypotheses,
        reinterpret_cast<const bitfusion::kernels::FinishedState*>(
            bufferCast<bitfusion::kernels::FinishedState::UnderlyingType>(*decodingOutput.finished)),
        bufferCast<float>(*decodingOutput.cumLogProbs), batchSize, beamWidth, stream.get());
    sync_check_cuda_error();

    bitfusion::kernels::invokeFinalize(bufferCast<TokenIdType>(finalOutputIds),
...
void IDecoder::acceptDraftTokensByIds(const ITensor& targetTokenIds, const ITensor& draftTokenIds,
    ...
    ITensor& finishedFinal, ITensor& finishedSum, BufferManager::CudaStreamPtr const& stream)
{
    ...
    bitfusion::kernels::invokeAcceptDraftTokensByIds(bufferCast<SizeType>(draftTokenIds),
        ...
        reinterpret_cast<const bitfusion::kernels::FinishedState*>(
            bufferCast<bitfusion::kernels::FinishedState::UnderlyingType>(finishedVec)),
        reinterpret_cast<bitfusion::kernels::FinishedState*>(
            bufferCast<bitfusion::kernels::FinishedState::UnderlyingType>(finishedFinal)),
...
void IDecoder::acceptDraftTokensByLogits(ITensor& draftLogits, const ITensor& targetLogits, ITensor& draftProbs,
    ...
    BufferManager::CudaStreamPtr const& stream)
{
    ...
    {
        bitfusion::kernels::acceptDraftTokensByLogits(bufferCast<float>(draftLogits),
            ...
            reinterpret_cast<bitfusion::kernels::FinishedState*>(
                bufferCast<bitfusion::kernels::FinishedState::UnderlyingType>(finished)),
    ...
    {
        bitfusion::kernels::acceptDraftTokensByLogits(bufferCast<half>(draftLogits),
