...
DecoderBatch::DecoderBatch(
    std::size_t vocabSize, std::size_t vocabSizePadded, DecoderBatch::CudaStreamPtr stream)
...
void DecoderBatch::setup(SizeType maxBatchSize, SizeType maxBeamWidth, SizeType maxAttentionWindow,
    SizeType maxSequenceLength, SizeType maxTokensPerStep, nvinfer1::DataType dtype)
{
    ...
    for (SizeType i = 0; i < maxBatchSize; ++i)
    {
        ...
        mDecoders[i] = IDecoder::create(dtype, mVocabSize, mVocabSizePadded, mStreams[i]);
...
DecoderBatch::TokenPtr DecoderBatch::forwardAsync(
    decoder_batch::Output& output, decoder_batch::Input const& input)
{
    ...
    for (std::int32_t bi = 0; bi < mActualBatchSize; ++bi)
    {
        ...
        if (mGeneratedTokensPerStep[bi] > 1 && mAcceptByLogits[bi])
        {
            ...
            IDecoder::acceptDraftTokensByLogits(
 *mDraftLogits[bi],
 *targetLogits,
 *draftProbs,
 *targetProbs,
 ...
 *finishedSteps, static_cast<SizeType>(mVocabSize),
        ...
        if (mGeneratedTokensPerStep[bi] > 1 && !mAcceptByLogits[bi])
        {
            ...
            IDecoder::acceptDraftTokensByIds(
 *dOutput.ids,
 *draftTokenIds,
 *dInput.lengths,
 *numDraftTokens,
...
void DecoderBatch::forwardSync(decoder_batch::Token const& token)
{
    LOG_DEBUG("%s start", __PRETTY_FUNCTION__);
...
CudaEvent DecoderBatch::postProcessRequest(SizeType batchIdx) const
{
    LOG_DEBUG("%s start", __PRETTY_FUNCTION__);
    auto& stream = mStreams[batchIdx];
...
void DecoderBatch::forwardAsync(decoder::Output& output, decoder::Input const& input)
{
    LOG_DEBUG("%s start", __PRETTY_FUNCTION__);
...
void DecoderBatch::finalize() const
{
    LOG_DEBUG("%s start", __PRETTY_FUNCTION__);
...
CudaEvent DecoderBatch::finalize(SizeType batchIdx) const
{
    LOG_DEBUG("%s start", __PRETTY_FUNCTION__);
