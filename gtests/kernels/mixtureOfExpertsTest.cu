...
protected:
    ...
    bitfusion::ActivationType mActType = bitfusion::ActivationType::Relu;
...
#undef PRINT
    ...
    {
        if (mActType == bitfusion::ActivationType::Identity)
            return in;
        if (mActType == bitfusion::ActivationType::Relu)
        ...
        std::vector<DataType> scale_probs = {})
    {
        ...
        for (int token_id = 0; token_id < mTotalTokens; token_id++)
        {
            for (int k_idx = 0; k_idx < mK; k_idx++)
            {
                ...
                if (selected_expert < mNumExperts)
                {
                    ASSERT_FLOAT_EQ(
...
{
    mNormMode = bitfusion::kernels::MOEExpertScaleNormalizationMode::RENORMALIZE;
    BasicPermuteTest();
    BasicPermuteTest(2);
    BasicPermuteTest(3);
}
...
TEST_F(MixtureOfExpertsTest, Finished)
{
    ...
    EXPECT_EQ(selected_expert[4], num_experts);
