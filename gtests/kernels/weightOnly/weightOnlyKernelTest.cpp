...
using bitfusion::kernels::WeightOnlyParams;
using bitfusion::kernels::WeightOnlyType;
using bitfusion::kernels::WeightOnlyQuantType;
using bitfusion::kernels::WeightOnlyActivationType;
using bitfusion::kernels::WeightOnlyActivationFunctionType;
...
template <typename T>
std::vector<bitfusion::cutlass_extensions::CutlassGemmConfig> get_configs(T& runner, int k)
{
    auto configs = runner.getConfigs();
    std::vector<bitfusion::cutlass_extensions::CutlassGemmConfig> rets;
    for (auto config : configs)
    {
        ...
        if (config.split_k_style != bitfusion::cutlass_extensions::SplitKStyle::NO_SPLIT_K)
        {
    ...
    int k, int group_size, int warmup, int iter)
{
    ...
    if constexpr (std::is_same_v<KernelFlag, CudaKernel>)
    {
        ...
        {
            bitfusion::kernels::weight_only_batched_gemv_launcher(params, s);
        }
    ...
    {
        bitfusion::kernels::cutlass_kernels::CutlassFpAIntBGemmRunner<typename AType<AFlag>::CutlassKernelAType,
