...
using bitfusion::plugins::AllreducePluginCreator;
using bitfusion::plugins::AllreducePlugin;
using bitfusion::kernels::AllReduceStrategyType;
    ...
    const void* const* inputs, void* const* outputs, void* workspace, cudaStream_t stream) noexcept
{
    ...
    using bitfusion::common::datatype_enum;
    datatype_enum type;
    switch (mType)
    {
    ...
    else
    {
        ...
        auto params = bitfusion::kernels::AllReduceParams::deserialize(
            reinterpret_cast<const int32_t*>(inputs[1]), nRanks, myRank, mCounter);

        bitfusion::kernels::invokeMultiGpuBarrier(params, stream);
        cudaMemcpyAsync(
            params.peer_comm_buffer_ptrs[myRank], inputs[0], size * sizePerElem, cudaMemcpyDeviceToDevice, stream);

        bitfusion::kernels::customAllReduce(params, outputs[0], size, sizePerElem, type, runtimeStrategy, stream);
    }
