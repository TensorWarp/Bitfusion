...
using namespace bitfusion::plugins;
using namespace bitfusion::kernels;
using bitfusion::common::QuantMode;
using bitfusion::common::nextWorkspacePtr;
using bitfusion::common::calculateTotalWorkspaceSize;
using bitfusion::plugins::MixtureOfExpertsPluginCreator;
using bitfusion::plugins::MixtureOfExpertsPlugin;
...
MixtureOfExpertsPlugin::MixtureOfExpertsPlugin(int number_of_experts, int top_k, int expert_hidden_size,
    int expert_inter_size, bitfusion::ActivationType activation_type, nvinfer1::DataType type,
...
bitfusion::plugins::MixtureOfExpertsPlugin::MixtureOfExpertsPlugin(const MixtureOfExpertsPlugin& other)
    : mMOERunner()
...
auto MixtureOfExpertsPlugin::setupWorkspace(void* base_ptr, int num_tokens) const -> WorkspaceInfo
{
    ...
    size_t dtype_size = bitfusion::common::getDTypeSize(mType);
    ...
    cudaStream_t stream) noexcept
{
    ...
    const int num_not_finished = num_tokens;
...
MixtureOfExpertsPluginCreator::MixtureOfExpertsPluginCreator()
{
    ...
    mPluginAttributes.emplace_back(nvinfer1::PluginField(
        "activation_type", nullptr, PluginFieldType::kINT32, static_cast<int>(bitfusion::ActivationType::Identity)));
    mPluginAttributes.emplace_back(
    ...
    const char* name, const nvinfer1::PluginFieldCollection* fc) noexcept
{
    ...
    try
    {
        auto pluginProfiler = moePluginProfiler.createGemmPluginProfiler( false);
        auto* obj = new MixtureOfExpertsPlugin(
            mNumExperts, mK, mExpertHiddenSize, mExpertInterSize,
            static_cast<bitfusion::ActivationType>(mActivationType), static_cast<nvinfer1::DataType>(mType),
    ...
    const char* name, const void* serialData, size_t serialLength) noexcept
{
    try
    {
        auto pluginProfiler = moePluginProfiler.createGemmPluginProfiler( true);
        auto* obj = new MixtureOfExpertsPlugin(
...
std::vector<size_t> MixtureOfExpertsGemmProfiler::getProfilerWorkspaces(int maxM)
{
    ...
    size_t dtype_bytes = bitfusion::common::getDTypeSize(plugin.mType);
    size_t weight_bytes = bitfusion::common::getDTypeSize(plugin.mWeightType);
...
void MixtureOfExpertsGemmProfiler::computeTmpSize(int maxM, int n, int k)
{
    ...
    size_t bytes = bitfusion::common::calculateTotalWorkspaceSize(workspaces.data(), workspaces.size());
