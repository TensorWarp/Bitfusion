...
#include <cstdint>
#include <type_traits>
#include "../bertAttentionPlugin/bertAttentionPlugin.h"

using namespace nvinfer1;
using namespace bitfusion::kernels;
namespace tc = bitfusion::common;
using bitfusion::plugins::GPTAttentionPluginCreatorCommon;
using bitfusion::plugins::GPTAttentionPluginCommon;
...
bool GPTAttentionPluginCommon::convertMMHAParamsToXQAParams(
    bitfusion::kernels::XQAParams& xqaParams, const EnqueueGenerationParams<T, KVCacheBuffer>& generationsParams)
{
...
GPTAttentionPluginCommon::GPTAttentionPluginCommon(int num_heads, int num_kv_heads, int head_size, int unidirectional,
    float q_scaling, bitfusion::kernels::PositionEmbeddingType position_embedding_type,
    int rotary_embedding_dim,
...
size_t GPTAttentionPluginCommon::getWorkspaceSizeForContext(nvinfer1::DataType type, int32_t nbReq,
    int32_t input_seq_length, int32_t max_kv_cache_len, int32_t cross_qkv_length) const noexcept
{
    const int local_hidden_units_qo = mNumHeads * getHeadSize();
    const int local_hidden_units_kv = mNumKVHeads * getHeadSize();

    auto const size = bitfusion::runtime::BufferDataType(type).getSize();
    ...
    const size_t paged_kv_tma_desc_size = mPagedKVCache && mPagedContextFMHA
        ? batch_size * 2 * TMA_DESC_SIZE_IN_BYTE * tc::divUp(max_kv_cache_len, mTokensPerBlock)
        : 0;
...
size_t GPTAttentionPluginCommon::getWorkspaceSizeForGeneration(
    nvinfer1::DataType type, int32_t total_num_seq) const noexcept
{
    const int local_hidden_units_qo = mNumHeads * getHeadSize();
    const int local_hidden_units_kv = mNumKVHeads * getHeadSize();
...
int GPTAttentionPluginCommon::enqueueContext(const EnqueueContextParams<T, KVCacheBuffer>& params, cudaStream_t stream)
{
    ...
    const int attention_seq_len_1 = params.input_seq_length;
    const int attention_seq_len_2 = isCrossAttention() ? params.cross_qkv_length : params.input_seq_length;
        ...
        if (is_qk_buf_float_ == true)
        {
            if (isRelativePosition())
            {
                invokeAddRelativeAttentionBiasUnaligned(qk_buf_float_, relative_attention_bias, params.batch_size,
                    ...
                    max_distance > 0, relative_attention_bias_stride, max_distance, true);
            }
    ...
    const EnqueueGenerationParams<T, KVCacheBuffer>& params, cudaStream_t stream)
{
    ...
    {
        XQAParams xqaParams{};
        if (bitfusion::kernels::XQADispatchHelper<T, KVCacheBuffer>::CanSupport && mDecoderXQARunner.get() != nullptr
...
size_t GPTAttentionPluginCommon::getCommonSerializationSize() noexcept
{
    return sizeof(mNumHeads) + sizeof(mNumKVHeads) + sizeof(mHeadSize) + sizeof(mUnidirectional) + sizeof(mQScaling)
        ...
        + sizeof(mMultiBlockMode) + sizeof(unsigned int)
        ...
        + sizeof(mPagedContextFMHA) + sizeof(mUseKVCache);
}
