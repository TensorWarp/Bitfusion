    ...
    bool randomThreshold, float constantThreshold)
{
    ...
    bool shouldSkipDecoding = false;

    for (int vIdx = threadIdx.x; vIdx < vocabSize && !shouldSkipDecoding; vIdx += blockDim.x)
    {
        ...
        if (targetProb < threshold * draftProb)
        {
            shouldSkipDecoding = true;
        }
    }
}
...
__global__ void correctAcceptedStatesAndLogits(const T* draftProbs, T* targetProbs, T* targetLogits,
    const int* numsDraftTokens, int* acceptedTokens, int batchSize, int beamWidth, int vocabSize)
{
    const auto bid = blockIdx.x;
    const auto numDraftTokens = numsDraftTokens[bid];
    ...
    if (threadIdx.x == 0)
    {
        ...
        {
            const int finishedState = acceptedTokens[ti * batchSize * beamWidth + bid];
            bool localSkipDecoding = (finishedState == 0);
            if (!cummulativeSkipDecoding && localSkipDecoding)
            {
                numAcceptedTokens = ti;
            }

            acceptedTokens[ti * batchSize * beamWidth + bid] = cummulativeSkipDecoding ? 0 : 1;
...
void acceptDraftTokensByLogits(T* draftLogits, T* targetLogits, T* draftProbs, T* targetProbs,
    ...
    cudaStream_t stream)
{
    // Invoke softmax calculations (You can add error handling here if needed)
    invokeAddBiasSoftMax<T>(draftLogits, draftProbs, nullptr, nullptr, finished,
        batchSize * beamWidth * maxDraftTokens, vocabSize, vocabSizePadded, stream);

    invokeAddBiasSoftMax<T>(targetLogits, targetProbs, nullptr, nullptr, finished,
        batchSize * beamWidth * maxDraftTokens, vocabSize, vocabSizePadded, stream);

    // Define grid and block dimensions for the first kernel
    dim3 block(1024);
    dim3 grid(batchSize * beamWidth, maxDraftTokens);

    // Call the first CUDA kernel
    ...
    // Reset grid and block dimensions for the second kernel
    grid = dim3(batchSize * beamWidth, 1);

    // Define and allocate the acceptedTokens array on the device
    int* d_acceptedTokens;
    cudaMalloc((void**)&d_acceptedTokens, sizeof(int) * batchSize * beamWidth * maxDraftTokens);

    // Call the second CUDA kernel
    correctAcceptedStatesAndLogits << <grid, block >> > (draftProbs, targetProbs, targetLogits, numsDraftTokens, d_acceptedTokens,
        batchSize, beamWidth, vocabSize);

    // Free the memory allocated for d_acceptedTokens
    cudaFree(d_acceptedTokens);
}


// Explicit template instantiations for float and half
template void acceptDraftTokensByLogits<float>(float* draftLogits, float* targetLogits, float* draftProbs, float* targetProbs,
    const int* numsDraftTokens, FinishedState* finished, curandState_t* curandState, int batchSize, int beamWidth,
    int vocabSize, int vocabSizePadded, int maxDraftTokens, bool randomThreshold, float constantThreshold,
    cudaStream_t stream);

template void acceptDraftTokensByLogits<half>(half* draftLogits, half* targetLogits, half* draftProbs, half* targetProbs,
    const int* numsDraftTokens, FinishedState* finished, curandState_t* curandState, int batchSize, int beamWidth,
    int vocabSize, int vocabSizePadded, int maxDraftTokens, bool randomThreshold, float constantThreshold,
    cudaStream_t stream);
