...
void GpuBuffer<T>::Deallocate()
{
    ...
    if (status != cudaSuccess) {
        std::cerr << "GpuBuffer::Deallocate failed (cudaFree) (CUDA error: " << cudaGetErrorString(status) << ")" << std::endl;
        std::exit(1);
    }
    getGpu()._totalGPUMemory -= static_cast<AlignedULI>(_length * sizeof(T));
...
#ifdef MEMTRACKING
    std::cout << "Mem--: " << getGpu()._totalGPUMemory << " " << getGpu()._totalCPUMemory << std::endl;
#endif
}
...
void GpuBuffer<T>::Copy(T* pBuff)
{
    ...
    if (status != cudaSuccess) {
        std::cerr << "cudaMemcpy GpuBuffer::Copy failed (CUDA error: " << cudaGetErrorString(status) << ")" << std::endl;
        std::exit(1);
    }
}
...
void GpuBuffer<T>::Upload(const T* pBuff) const
{
    if (pBuff)
    {
        ...
        if (status != cudaSuccess) {
            std::cerr << "cudaMemcpy GpuBuffer::Upload failed (CUDA error: " << cudaGetErrorString(status) << ")" << std::endl;
            std::exit(1);
        }
    }
...
void GpuBuffer<T>::Download(T* pBuff)
{
    if (pBuff)
    {
        ...
        if (status != cudaSuccess) {
            std::cerr << "cudaMemcpy GpuBuffer::Download failed (CUDA error: " << cudaGetErrorString(status) << ")" << std::endl;
            std::exit(1);
        }
    }
    else if (_bSysMem && !_bManaged)
