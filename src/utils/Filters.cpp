                                     ...
                                     const std::string &filePath)
{
        ...
        while (getline(samplesFile, line))
        {
            ...
            if (sample != -1)
            {
                ...
                if (samplesFilterCount % gSamplesLoggingInterval == 0)
                {
                    ...
                    std::cout << "Time " << elapsed_seconds(start, end) << '\n';
    ...
    {
        std::cout << "Unable to read the file " << filePath << '\n';
                               ...
                               const std::string &filterFilePath)
{
    ...
    {
        std::cout << "Loading " << files.size() << " filter files" << '\n';

        for (auto const &file : files)
        {
            std::cout << "\tLoading filter: " << file << '\n';
            loadSingleFilter(xMInput, xMSamples, *samplefilters.get(), file);
        }
    }

    std::cout << "Info:SamplesFilter " << samplefilters->size() << '\n';
}
