    ...
    maxK(maxK)
{
    try
    {
        ...
        for (const Layer* layer : outputLayers)
        {
            ...
            if (maxK != ALL)
            {
                ...
                GpuBuffer<float>* outputScores = new GpuBuffer<float>(outputBufferLength, false, false);
    ...
    {
        std::cerr << "Error during Runtime initialization: " << ex.what() << '\n';
    }
}
    ...
    for (int i = 0; i < datasetDescriptors.size(); ++i)
    {
        ...
        auto future = threadPool.enqueue([this, descriptor]() -> DataSetBase* {
            try {
                ...
                std::cerr << "Error creating dataset: " << ex.what() << '\n';
                return nullptr;
            }
            });
