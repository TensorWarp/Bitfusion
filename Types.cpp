...
static std::map<DataSetEnums::Kind, string> sKindMap = {
    {DataSetEnums::Numeric, "Numeric"},
    {DataSetEnums::Image,   "Image"},
    {DataSetEnums::Audio,   "Audio"}
};

ostream& operator<< (ostream& out, DataSetEnums::Kind& k)
{
    out << sKindMap[k];
    return out;
}
...
static std::map<DataSetEnums::Attributes, string> sAttributesMap = {
...
ostream& operator<< (ostream& out, DataSetEnums::Attributes& a)
{
    out << sAttributesMap[a];
    return out;
}

static std::map<DataSetEnums::Sharding, string> sShardingMap = {
    {DataSetEnums::None,  "None"},
    {DataSetEnums::Model, "Model"},
    {DataSetEnums::Data,  "Data"}
};

ostream& operator<< (ostream& out, DataSetEnums::Sharding& s)
{
    out << sShardingMap[s];
    return out;
}
...
static std::map<DataSetEnums::DataType, string> sDataTypeMap = {
...
ostream& operator<< (ostream& out, DataSetEnums::DataType& t)
{
    out << sDataTypeMap[t];
    return out;
}
...
template<typename T> vector<tuple<uint64_t, uint64_t> > DataSet<T>::getMemoryUsage()
{
...
template<typename T> DataSet<T>::DataSet(uint32_t examples, const DataSetDimensions &dim, const string &name) :
...
template<typename T> DataSet<T>::DataSet(uint32_t examples, uint32_t uniqueExamples, const DataSetDimensions &dim,
                                             const string &name) :
...
template<typename T> DataSet<T>::DataSet(uint32_t examples, float sparseDensity, const DataSetDimensions &dim,
...
template<typename T> DataSet<T>::DataSet(uint32_t examples, uint32_t uniqueExamples, size_t sparseDataSize,
...
template<typename T> void DataSet<T>::LoadDenseData(const void *srcData)
{
...
template<typename T> void DataSet<T>::CopyDenseData(const void *srcData)
{
    const T* srcDataTyped = static_cast<const T*>(srcData);
...
template<typename T> void DataSet<T>::LoadSparseData(const uint64_t *srcSparseStart, const uint64_t *srcSparseEnd,
...
template<typename T> void DataSet<T>::CopySparseData(const uint64_t *srcSparseStart, const uint64_t *srcSparseEnd,
                                                       const void *srcSparseData, const uint32_t *srcSparseIndex)
{
    const T* srcSparseDataTyped = static_cast<const T*>(srcSparseData);
...
template<typename T> void DataSet<T>::LoadSparseData(const long *srcSparseStart, const long *srcSparseEnd,
...
template<typename T> void DataSet<T>::CopySparseData(const long *srcSparseStart, const long *srcSparseEnd,
                                                       const void *srcSparseData, const long *srcSparseIndex)
{
    const T* srcSparseDataTyped = static_cast<const T*>(srcSparseData);
...
template<typename T> void DataSet<T>::LoadIndexedData(const uint32_t *srcIndexedData)
{
...
template<typename T> void DataSet<T>::LoadDataWeight(const float *srcWeightData)
{
...
template<typename T> T DataSet<T>::GetDataPoint(uint32_t n, uint32_t x, uint32_t y, uint32_t z)
{
    if (_attributes & DataSetEnums::Sparse)
    {
        if (getGpu()._id == 0)
        {
            printf("DataSet::GetDataPoint: attempt to read non-sparse data from sparse data set.\n");
        }
        getGpu().Shutdown();
        exit(-1);
    }
    ...
    if (n >= _examples)
    {
        if (getGpu()._id == 0)
        {
            printf("DataSet::GetDataPoint: illegal example index.\n");
        }
        getGpu().Shutdown();
        exit(-1);
    }
    ...
    if ((x >= _width) || (y >= _height) || (z >= _length))
    {
        if (getGpu()._id == 0)
        {
            printf("DataSet::GetDataPoint: illegal datapoint coordinates (%u, %u, %u).\n", x, y, z);
        }
        getGpu().Shutdown();
        exit(-1);
    }
...
template<typename T> bool DataSet<T>::SetDataPoint(T v, uint32_t n, uint32_t x, uint32_t y, uint32_t z)
{
    if (_attributes & DataSetEnums::Sparse)
    {
        if (getGpu()._id == 0)
        {
            printf("DataSet::SetDataPoint: attempt to read non-sparse data from sparse data set.\n");
        }
        getGpu().Shutdown();
        exit(-1);
    }
    ...
    if (n >= _examples)
    {
        if (getGpu()._id == 0)
        {
            printf("DataSet::SetDataPoint: illegal example index.\n");
        }
        getGpu().Shutdown();
        exit(-1);
    }
    ...
    if ((x >= _width) || (y >= _height) || (z >= _length))
    {
        if (getGpu()._id == 0)
        {
            printf("DataSet::SetDataPoint: illegal datapoint coordinates (%u, %u, %u).\n", x, y, z);
        }
        getGpu().Shutdown();
        exit(-1);
    }
...
template<typename T> uint64_t DataSet<T>::GetSparseDataPoints(uint32_t n)
{
    if (!(_attributes & DataSetEnums::Sparse))
    {
        if (getGpu()._id == 0)
        {
            printf("DataSet::GetSparseDataPoints: attempt to read sparse data from non-sparse data set.\n");
        }
        getGpu().Shutdown();
        exit(-1);
    }
    ...
    if (n >= _examples)
    {
        if (getGpu()._id == 0)
        {
            printf("DataSet::GetSparseDataPoints: illegal example index.\n");
        }
        getGpu().Shutdown();
        exit(-1);
    }
...
template<typename T> uint32_t DataSet<T>::GetSparseIndex(uint32_t n, uint32_t i)
{
    if (!(_attributes & DataSetEnums::Sparse))
    {
        if (getGpu()._id == 0)
        {
            printf("DataSet::GetSparseIndex: attempt to read sparse data from non-sparse data set.\n");
        }
        getGpu().Shutdown();
        exit(-1);
    }
    ...
    if (n >= _examples)
    {
        if (getGpu()._id == 0)
        {
            printf("DataSet::GetSparseIndex: illegal example index.\n");
        }
        getGpu().Shutdown();
        exit(-1);
    }
    ...
    if (i >= _vSparseEnd[n] - _vSparseStart[n])
    {
        if (getGpu()._id == 0)
        {
            printf("DataSet::GetSparseIndex: Sparse index %u out of range (0, %lu).\n", i, _vSparseEnd[n] - _vSparseStart[n]);
        }
        getGpu().Shutdown();
        exit(-1);
    }
...
template<typename T> bool DataSet<T>::SetSparseIndex(uint32_t n, uint32_t i, uint32_t v)
{
    if (!(_attributes & DataSetEnums::Sparse))
    {
        if (getGpu()._id == 0)
        {
            printf("DataSet::SetSparseDataIndex: attempt to read sparse data from non-sparse data set.\n");
        }
        getGpu().Shutdown();
        exit(-1);
    }
    ...
    if (n >= _examples)
    {
        if (getGpu()._id == 0)
        {
            printf("DataSet::SetSparseDataIndex: illegal example index.\n");
        }
        getGpu().Shutdown();
        exit(-1);
    }
    ...
    if (i >= _vSparseEnd[n] - _vSparseStart[n])
    {
        if (getGpu()._id == 0)
        {
            printf("DataSet::SetSparseIndex: Sparse index %u out of range (0, %lu).\n", i, _vSparseEnd[n] - _vSparseStart[n]);
        }
        getGpu().Shutdown();
        exit(-1);
    }
...
template<typename T> T DataSet<T>::GetSparseDataPoint(uint32_t n, uint32_t i)
{
    if (!(_attributes & DataSetEnums::Sparse))
    {
        if (getGpu()._id == 0)
        {
            printf("DataSet::GetSparseDataPoint: attempt to read sparse data from non-sparse data set.\n");
        }
        getGpu().Shutdown();
        exit(-1);
    }
    ...
    if (n >= _examples)
    {
        if (getGpu()._id == 0)
        {
            printf("DataSet::GetSparseDataPoint: illegal example index.\n");
        }
        getGpu().Shutdown();
        exit(-1);
    }
    ...
    if (i >= _vSparseEnd[n] - _vSparseStart[n])
    {
        if (getGpu()._id == 0)
        {
            printf("DataSet::GetSparseDataPoint: Sparse index %u out of range (0, %lu).\n", i, _vSparseEnd[n] - _vSparseStart[n]);
        }
        getGpu().Shutdown();
        exit(-1);
    }
...
template<typename T> bool DataSet<T>::SetSparseDataPoint(uint32_t n, uint32_t i, T v)
{
    if (!(_attributes & DataSetEnums::Sparse))
    {
        if (getGpu()._id == 0)
        {
            printf("DataSet::SetSparseDataPoint: attempt to read sparse data from non-sparse data set.\n");
        }
        getGpu().Shutdown();
        exit(-1);
    }
    ...
    if (n >= _examples)
    {
        if (getGpu()._id == 0)
        {
            printf("DataSet::SetSparseDataPoint: illegal example index.\n");
        }
        getGpu().Shutdown();
        exit(-1);
    }
    ...
    if (i >= _vSparseEnd[n] - _vSparseStart[n])
    {
        if (getGpu()._id == 0)
        {
            printf("DataSet::SetSparseDataPoint: Sparse index %u out of range (0, %lu).\n", i, _vSparseEnd[n] - _vSparseStart[n]);
        }
        getGpu().Shutdown();
        exit(-1);
    }
...
template<typename T> DataSet<T>::DataSet(const string& fname, uint32_t n) :
_pbData(),
_pbSparseData()
{
    bool bResult                                = true;
    if (getGpu()._id == 0)
    {
        ...
        try
        {
            ...
            if (nameAtt.isNull())
            {
                std::cerr << ("NcException", "DataSet::DataSet: No dataset name supplied in NetCDF input file " + fname, __FILE__, __LINE__);
            }
            nameAtt.getValues(_name);
            cout << "DataSet<T>::DataSet: Name of data set: " << _name << endl;
            ...
            {
                std::cerr << ("NcException", "DataSet::DataSet: No datatype supplied in NetCDF input file " + fname, __FILE__, __LINE__);
            }
            int dataType;
            ...
            {
                std::cerr << ("NcException", "DataSet::DataSet: No attributes supplied in NetCDF input file " + fname, __FILE__, __LINE__);
            }
            ...
            if (_attributes != 0)
            {
                ...
                int position = 0;
                cout << "DataSet<T>::DataSet: Attributes:";
                while (tempAtt != 0)
                {
                    if (tempAtt & 1)
                    {
            ...
            {
                std::cerr << ("NcException", "DataSet::DataSet: No examples count supplied in NetCDF input file " + fname, __FILE__, __LINE__);
            }
            _examples                           = examplesDim.getSize();

            if (_examples == 0)
            {
                std::cerr << ("NcException", "DataSet::DataSet: Zero-valued Examples count in NetCDF input file " + fname, __FILE__, __LINE__);
            }
            ...
            {
                std::cerr << ("NcException", "DataSet::DataSet: No dimension count supplied in NetCDF input file " + fname, __FILE__, __LINE__);
            }
            dimensionsAtt.getValues(&_dimensions);

            if ((_dimensions < 1) || (_dimensions > 3))
            {
                std::cerr << ("NcException", "DataSet::DataSet: Invalid dimension count (" + to_string(_dimensions) + ") supplied in NetCDF input file " + fname, __FILE__, __LINE__);
            }
            ...
            if (widthAtt.isNull())
            {
                std::cerr << ("NcException", "DataSet::DataSet: No datapoint width supplied in NetCDF input file " + fname, __FILE__, __LINE__);
            }
            ...
            if (_dimensions > 1)
            {
                ...
                if (heightAtt.isNull())
                {
                    std::cerr << ("NcException", "DataSet::DataSet: No datapoint height supplied in NetCDF input file " + fname, __FILE__, __LINE__);
                }
            ...
            if (_dimensions > 2)
            {
                ...
                if (lengthAtt.isNull())
                {
                    std::cerr << ("NcException", "DataSet::DataSet: No datapoint length supplied in NetCDF input file " + fname, __FILE__, __LINE__);
                }
                lengthAtt.getValues(&_length);
            }
            else
                _length                         = 1;
            cout << "DataSet<T>::DataSet: " << _dimensions << "-dimensional data comprised of (" << _width << ", " << _height << ", " << _length << ") datapoints." << endl;

            if ((_width == 0) || (_height == 0) || (_length == 0))
            {
                std::cerr << ("NcException", "DataSet::DataSet: Invalid dataset dimensions in NetCDF input file " + fname, __FILE__, __LINE__);
            }
            ...
            if (_attributes & DataSetEnums::Sparse)
            {
                ...
                {
                    std::cerr << ("NcException", "DataSet::DataSet: No sparse data dimensions supplied in NetCDF input file " + fname, __FILE__, __LINE__);
                }
                _sparseDataSize                 = sparseDataDim.getSize();

                if (_sparseDataSize == 0)
                {
                    std::cerr << ("NcException", "DataSet::DataSet: Sparse data set with no actual data in NetCDF input file " + fname, __FILE__, __LINE__);
                }

                _vSparseIndex.resize(_sparseDataSize);
                cout << "DataSet<T>::DataSet: " << _sparseDataSize << " total datapoints." << endl;
                vname                           = "sparseStart" + nstring;
                NcVar sparseStartVar            = nfc.getVar(vname);
                if (sparseStartVar.isNull())
                {
                    std::cerr << ("NcException", "DataSet::DataSet: No sparse offset start supplied in NetCDF input file " + fname, __FILE__, __LINE__);
                }
                vname                           = "sparseEnd" + nstring;
                NcVar sparseEndVar              = nfc.getVar(vname);
                if (sparseEndVar.isNull())
                {
                    std::cerr << ("NcException", "DataSet::DataSet: No sparse data end supplied in NetCDF input file " + fname, __FILE__, __LINE__);
                }
                vname                           = "sparseIndex" + nstring;
                NcVar sparseIndexVar            = nfc.getVar(vname);
                if (sparseIndexVar.isNull())
                {
                    std::cerr << ("NcException", "DataSet::DataSet: No sparse data indices supplied in NetCDF input file " + fname, __FILE__, __LINE__);
                }
                ...
                if (!(_attributes & DataSetEnums::Boolean))
                {
                    ...
                    {
                        std::cerr << ("NcException", "DataSet::DataSet: No sparse data located in NetCDF input file " + fname, __FILE__, __LINE__);
                    }
            ...
            else
            {
                ...
                if (dataDim.isNull())
                {
                        std::cerr << ("NcException", "DataSet::DataSet: No data dimensions located in NetCDF input file " + fname, __FILE__, __LINE__);
                }
                vname                           = "data" + nstring;
            ...
            if (_attributes & DataSetEnums::Weighted)
            {
                ...
                {
                    std::cerr << ("NcException", "DataSet::DataSet: No data weights located in NetCDF input file " + fname, __FILE__, __LINE__);
                }
                _vDataWeight.resize(_examples);
            ...
            if (_attributes & DataSetEnums::Indexed)
            {
                ...
                if (indexVar.isNull())
                {
                    std::cerr << ("NcException", "DataSet::DataSet: No indexed data located in NetCDF input file " + fname, __FILE__, __LINE__);
                }
               _vIndex.resize(_examples);
               indexVar.getVar(_vIndex.data());
            }

            cout << "DataSet<T>::DataSet: " << _examples << " examples." << endl;
            cout << "DataSet<T>::DataSet: " << _uniqueExamples << " unique examples." << endl;
        }
        catch (NcException& e)
        {
            ...
            if (!bOpened)
            {
                cout << "Exception: DataSet::DataSet: Error opening NetCDF input file " << fname << endl;
            }
            else
            {
...
template<typename T> bool DataSet<T>::Rename(const string& name)
{
    _name                                       = name;
    return true;
}

template<typename T> bool DataSet<T>::CalculateSparseDatapointCounts()
{
    if (_attributes & DataSetEnums::Sparse)
    {
        ...
        for (size_t i = 0; i < _uniqueExamples; i++)
        {
            ...
            for (size_t j = _vSparseStart[i]; j < _vSparseEnd[i]; j++)
            {
                ...
                {
                    cout << "DataSet::CalculateSparseDatapointCounts: vCount address = " << _vSparseIndex[j] << " >= vCount size = " << N << endl;
    ...
    else
    {
        if (getGpu()._id == 0)
        {
            printf("DataSet::CalculateSparseDatapointCounts: Attempt to calculate sparse datapoint counts on non-sparse dataset %s.\n", _name.c_str());
        }
        return false;
    }
}

template<typename T> bool DataSet<T>::GenerateSparseTransposedMatrix(uint32_t batch, Layer* pLayer)
{
    ...
    if (offset > _sparseTransposedIndices)
    {
        ...
        {
            printf("DataSet::GenerateSparseTransposedMatrix: Allocating %lu bytes for sparse transposed weight gradient value matrix %s.\n", _sparseTransposedIndices * sizeof(float), _name.c_str());
...
template<typename T> bool DataSet<T>::SetDenoising(bool flag)
{
    if (!(_attributes & DataSetEnums::Sparse))
    {
        if (getGpu()._id == 0)
        {
            printf("DataSet::SetDenoising: Attempt to set denoising on non-sparse data set.\n");
        }
        return false;
    }
...
template<typename T> bool DataSet<T>::SetStreaming(bool flag)
{
    if (!getGpu()._bUnifiedMemory)
    {
        printf("DataSet::SetStreaming: Streaming datasets not supported on GPU %d\n", getGpu()._id);
    }
...
template<typename T> bool DataSet<T>::GetStreaming()
{
    return _bStreaming;
}

template<typename T> bool DataSet<T>::GenerateDenoisingData()
{
    if (!(_attributes & DataSetEnums::Sparse))
    {
        if (getGpu()._id == 0)
        {
            printf("DataSet::GenerateDenoisingData: Attempt to generate denoising randoms on non-sparse data set.\n");
        }
        return false;
    }
...
template<typename T> bool DataSet<T>::UnShard()
{
...
template<typename T> bool DataSet<T>::Shard(DataSetEnums::Sharding sharding)
{
    ...
    if (sharding == DataSetEnums::Model)
    {
        ...
        if (_attributes & DataSetEnums::Sparse)
        {
            if (getGpu()._id == 0)
            {
                printf("DataSet<T>::Shard: Model Sharding sparse dataset %s across all GPUs.\n", _name.c_str());
    ...
    else if (sharding == DataSetEnums::Data)
    {
        ...
        if (_attributes & DataSetEnums::Sparse)
        {
            if (getGpu()._id == 0)
            {
                printf("DataSet<T>::Shard: Data Sharding sparse dataset %s across all GPUs.\n", _name.c_str());
                for (size_t i = 1; i < getGpu()._numprocs; i++)
                {
        ...
        else
        {
            if (getGpu()._id == 0)
            {
                printf("DataSet<T>::Shard: Data Sharding dataset %s across all GPUs.\n", _name.c_str());
                for (size_t i = 1; i < getGpu()._numprocs; i++)
                {
                    size_t localExamples                = segment + (remainder > i);
...
bool DataSet<T>::SaveNetCDF(const std::string& fname)
{
    ...
    if (getGpu()._id == 0)
    {
        try
        {
            ...
            if (!bResult)
            {
                std::cerr << "SaveNetCDF: Unable to write dataset to NetCDF file " << fname << std::endl;
            }
        }
        catch (const netCDF::exceptions::NcException& e)
        {
            std::cerr << "SaveNetCDF: Unable to create or write to NetCDF output file " << fname << std::endl;
...
template<typename T>
bool DataSet<T>::WriteNetCDF(NcFile& nfc, const std::string& fname, const uint32_t n)
{
    bool bResult = true;
    try {
        if (getGpu()._id == 0)
        {
            ...
            auto createAttribute = [&](const std::string& attrName, const std::string& errMsg) {
                NcGroupAtt attribute = nfc.putAtt(attrName, _name);
                if (attribute.isNull())
                {
                    std::cerr << "NcException: " << errMsg << " " << fname << " (" << __FILE__ << ", " << __LINE__ << ")" << std::endl;
                    bResult = false;
                }
                };
            ...
            auto createDimension = [&](const std::string& dimName, size_t dimSize, const std::string& errMsg) {
                NcDim dimension = nfc.addDim(dimName, dimSize);
                if (dimension.isNull())
                {
                    std::cerr << "NcException: " << errMsg << " " << fname << " (" << __FILE__ << ", " << __LINE__ << ")" << std::endl;
                    bResult = false;
            ...
            auto createVariable = [&](const std::string& varName, const std::string& varType, const std::string& dimName, const void* data, const std::string& errMsg) {
                NcVar variable = nfc.addVar(varName, varType, dimName);
                if (variable.isNull())
                {
                    std::cerr << "NcException: " << errMsg << " " << fname << " (" << __FILE__ << ", " << __LINE__ << ")" << std::endl;
                    bResult = false;
...
bool SaveNetCDF(const std::string& fname, std::vector<DataSetBase*>& vDataSet) {
    ...
    try {
        if (mpiRank == 0) {
            ...
            netCDF::NcGroupAtt datasetsAtt = nfc.putAtt("datasets", ncUint, static_cast<unsigned int>(vDataSet.size()));
    ...
    if (!bResult) {
        getGpu().Shutdown();
        std::cerr << "Error: The MPI broadcast failed." << std::endl;
    }
...
std::vector<DataSetBase*> LoadNetCDF(const std::string& fname) {
                ...
                for (uint32_t i = 0; i < datasets; i++) {
                    ...
                    if (!rnc.getAtt(vname).isNull()) {
                        ...
                        case DataSetEnums::Char:
                            ...
                            break;
                        default:
                            std::cerr << "LoadNetCDF: Invalid data type in binary input file " << fname << std::endl;
                        }
                    }
                    else {
                        std::cerr << "NcException: LoadNetCDF: No " << vname << " attribute located in NetCDF input file " << fname << std::endl;
                    }
                }
            }
            else {
                std::cerr << "NcException: LoadNetCDF: No datasets count supplied in NetCDF input file " << fname << std::endl;
            }
        }
        catch (const netCDF::exceptions::NcException& e) {
            std::cerr << "NcException: LoadNetCDF: " << e.what() << std::endl;
            bResult = false;
        }
    }
    ...
    for (int i = 0; i < vDataType.size(); i++) {
        ...
        case DataSetEnums::RGB8:
            ...
            break;
        default:
            std::cerr << "LoadNetCDF: invalid dataset type in binary input file " << fname << std::endl;
            getGpu().Shutdown();
            std::exit(-1);
        }
...
vector<DataSetBase*> LoadImageData(const string& fname) {
    // TODO: Implement the loading logic
    return vector<DataSetBase*>();
}

vector<DataSetBase*> LoadCSVData(const string& fname) {
    // TODO: Implement the loading logic
    return vector<DataSetBase*>();
}

vector<DataSetBase*> LoadJSONData(const string& fname) {
    // TODO: Implement the loading logic
    return vector<DataSetBase*>();
}

vector<DataSetBase*> LoadAudioData(const string& name) {
    // TODO: Implement the loading logic
    return vector<DataSetBase*>();
}
